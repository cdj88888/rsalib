...
long long rsa_modExp(long long b, long long e, long long m)
{
	...
	if(e % 2 == 1) {
		return (b * rsa_modExp(b * rsa_modExp(b, (e - 1), m) % m);
	}
}
...
void rsa_gen_key(struct public_key_class *pub, struct private_key_class *priv, char *PRIME_SOURCE_FILE)
{
	FILE *primes_list;
	...
	do{
		int_bytes_read = fread(buffer, 1, sizeof(buffer) - 1, primes_list);
		buffer[bytes_read] = '\0';
		for (i=0;buffer[i];i++) {
			if(buffer[i] == '\n') {
				prime_count++;
...
long long *rsa_encrypted(const char *message, const unsigned long message_size, const struct public_key_class *pub)
{
...
char *rsa_decrypt(const long long *message, const unsigned long message_size, const struct private_key_class *priv)
{
  ...
  if((decrypt == NULL) || (temp == NULL)) {
    fprintf(stderr, "Error: Heap allocation failed.\n");
    return NULL;
  }
  //Now we go through each 8-bytes chunk and decrypt it.
  long long i = 0;
